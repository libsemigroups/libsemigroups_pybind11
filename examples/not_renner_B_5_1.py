...
p1 = examples.not_renner_type_B_monoid(5, 1)
q1 = examples.renner_type_B_monoid(5, 1)
...
twosided_congs = Sims2(p1)
wg1 = twosided_congs.find_if(4096, lambda wg: check_incompat(wg, q1))
word_graph.number_of_nodes_reachable_from(wg1, 0)  # 2177
wg1.induced_subgraph(0, 2177)
witness(wg1, q1)  # ([5, 0, 1, 2, 3, 4, 0, 1, 2, 3, 0, 1, 2, 0, 1, 0, 5], [10])
# meaning that the paths with source 0 labelled by these two words should lead to the same
# node but do not.
word_graph.follow_path(wg1, 0, [5, 0, 1, 2, 3, 4, 0, 1, 2, 3, 0, 1, 2, 0, 1, 0, 5])  # 6
word_graph.follow_path(wg1, 0, [10])  # 7
