...
if on_rtd:
    subprocess.call(
        "conda init bash && cd ../.. && conda active latest && python3 -m pip install -r requirements.txt && python3 -m pip install --use-feature=in-tree-build . && etc/make-doc-sphinx.sh",
        shell=True,
    )
