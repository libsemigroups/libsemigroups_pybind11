...
``libsemigroups_pybind11`` objects into :any:`InversePresentation`
...
be used to construct :any:`InversePresentation` objects. The following
...
Additionally, specify the following for *Return*:
    ...
    * ``InversePresentation`` for constructing an
      :any:`InversePresentation` over words of the same type as those in
      *p*.

This function returns an :any:`InversePresentation` with rules equivalent
to those of the input :any:`Presentation` *p*, but over a normalised
alphabet. If the alphabet of *p* is :math:`\{a_0, a_1, \dots, a_{n-1}\}`, then
the alphabet of the returned :any:`InversePresentation` will be
...
To construct an :any:`InversePresentation` from an
:any:`InversePresentation`, specify the following values for *args*:

    * **ip** (:any:`InversePresentation`) -- the
      :any:`InversePresentation` to convert.
...
the type of words specified in *Return*:
    ...
    #. When the type of words in *ip* and type of words specified in *Return*
       ...
       :any:`InversePresentation` equivalent to the input
       :any:`InversePresentation` *ip* but with words a different type
    ...
    #. When the type of words in *ip* and type of words specified in *Return*
       ...
       objects of type :any:`InversePresentation` must be converted from
...
To construct a :any:`InversePresentation` from a
:any:`InversePresentation` using a custom letter conversion function,
...
This function returns an :any:`InversePresentation` equivalent to the
input :any:`InversePresentation` *ip* but over words with letters of a
...
:any:`InversePresentation` to the other.
