...
def test_helpers_are_rules_sorted_025():
    ...
    p.alphabet("aAbBcCe")
    presentation.add_inverse_rules(p, "AaBbCce", "e")
    assert p.rules == ["aA", "e", "Aa", "e", "bB", "e", "Bb", "e", "cC", "e", "Cc", "e"]
...
def test_greedy_reduce_length_029():
    ...
    assert p.rules == ["dddd", "a", "cccc", "b", "abb", "baa", "c", "bbbb", "d", "aaaa"]
...
def test_presentation_balance_1_arg():
    p = examples.symmetric_group_Car56(8)
    assert p.rules == [
        [0, 0],
        [],
        [1, 1],
...
def test_presentation_balance_2_arg():
    p = Presentation("ab")
    p.contains_empty_word(True)
    p.rules = ["ab", "", "aaaaaaaaaaaaaa", ""]

    presentation.balance(p)
    assert p.rules == ["ab", "", "aaaaaaaaaaaaaa", ""]

    presentation.balance(p, "ba")
    assert p.rules == ["ab", "", "aaaaaaa", "bbbbbbb"]


def test_presentation_balance_3_arg():
    p = Presentation("abc")
    p.contains_empty_word(True)
    p.rules = ["aaaaaaaaaaaaaa", "", "caccacacacaccacacbbbb", "babbbcbcab"]

    presentation.balance(p, "ac", "ac")
    assert p.rules == ["aaaaaaa", "aaaaaaa", "cacacaccacacbbbb", "accacbabbbcbcab"]
