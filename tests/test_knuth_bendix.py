...
def test_knuth_bendix_return_policy():
    ...
    assert kb.check_confluence_interval(LIMIT_MAX) is kb.check_confluence_interval(LIMIT_MAX)
    assert kb.max_overlap(POSITIVE_INFINITY) is kb.max_overlap(POSITIVE_INFINITY)
    assert kb.max_rules(POSITIVE_INFINITY) is kb.max_rules(POSITIVE_INFINITY)
    assert kb.overlap_policy() is not kb.overlap_policy()
    assert kb.overlap_policy(kb.options.overlap.ABC) is kb.overlap_policy(kb.options.overlap.ABC)
